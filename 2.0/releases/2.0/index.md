...
**If you used the shipped Traefik labels.** `compose.yml` no longer carries them and no longer builds from the checkout. The reverse-proxy configuration moved to an overlay you copy from [Docker deployment](https://pvliesdonk.github.io/paperless-mcp/2.0/deployment/docker/index.md). Worth knowing while you migrate: the labels never routed as shipped, because the file declared no network for Traefik to join.
