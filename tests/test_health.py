...
def _kv_store_in_tmp(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    """
    monkeypatch.setenv("PAPERLESS_MCP_KV_STORE_URL", f"file://{tmp_path / 'kv'}")
...
def test_liveness_answers_outside_the_mcp_mount() -> None:
    ...
    assert body["server"]["name"] == "paperless-mcp"
...
def test_health_prefix_follows_the_mount_path() -> None:
    ...
    """
    mount = "/paperless-mcp/mcp"
    server = make_server(transport="http", http_path=mount)
    assert _get(server, "/paperless-mcp/health", http_path=mount).status_code == 200
...
def test_mount_path_env_var_is_the_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    """Without an explicit ``http_path``, ``PAPERLESS_MCP_HTTP_PATH`` decides.
    ...
    """
    mount = "/paperless-mcp/mcp"
    monkeypatch.setenv("PAPERLESS_MCP_HTTP_PATH", mount)
