...
def test_instructions_name_the_public_instance(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # No resource URI belongs in model-facing instructions: resources/read is a
    # client-to-server request, so the model can act on none of them, and every
    # variant worth acting on has a tool twin
    # (docs/design/reference/mcp-resource-access.md).
