...
def test_no_prefixed_env_var_is_interpolated(template_owned: str) -> None:
    """``compose.yml`` used to interpolate ``PAPERLESS_MCP_HOST`` into a
    ...
    Compose interpolates from the same ``.env``, so any ``${PAPERLESS_MCP_*}``
    ...
    """
    found = re.findall(
        r"\$\{?" + re.escape("PAPERLESS_MCP") + r"_[A-Z0-9_]+", template_owned
    )
    assert not found, (
