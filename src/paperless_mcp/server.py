    ...
    *,
    ...
    Args:
        ...
        http_path: The MCP mount path the caller will hand to
            ...
            same ``PAPERLESS_MCP_HTTP_PATH``-or-``/mcp`` fallback the
    ...
    # defaults to `PAPERLESS_MCP_SERVER_NAME` (falling back to the project
    ...
    instructions_for(mcp).documentation(
        "https://pvliesdonk.github.io/paperless-mcp/latest/llms.txt"
    )
    ...
    # is no HTTP app at all.  ``PAPERLESS_MCP_HEALTH_DETAIL`` (``status``
    ...
    # PAPERLESS_MCP_INSTANCE_DESCRIPTION supplies operator routing,
    # PAPERLESS_MCP_INSTRUCTIONS_EXTRA supplies operator policy, and the
    # legacy PAPERLESS_MCP_INSTRUCTIONS full replacement still wins with a
