...
When Paperless cannot answer (it is down, the URL is wrong, or the token is rejected), the `paperless` block is `{"version": null}` and the rest of the response is unaffected. The version is extra information about a call whose job is reporting this server's build, so it never fails that call.
