...
For OIDC auth variables, see [Authentication](https://pvliesdonk.github.io/paperless-mcp/1.0/guides/authentication/index.md).

Running behind a reverse proxy on a path prefix (`https://mcp.example.com/myservice/mcp`) rather than its own hostname needs two routing rules, one of which sits outside the prefix: see [Subpath Deployments](https://pvliesdonk.github.io/paperless-mcp/1.0/deployment/oidc/#subpath-deployments).
