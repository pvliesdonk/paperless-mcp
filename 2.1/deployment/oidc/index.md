...
This is the shipped `compose.yml` with a reverse proxy added. The service joins an external `traefik` network and carries router labels. It publishes no host port, because the proxy reaches it over that network instead. See [Docker](https://pvliesdonk.github.io/paperless-mcp/2.1/deployment/docker/index.md) for the base file and the same overlay without OIDC.
...
```
NS="container:$(docker compose ps -q paperless-mcp)"
