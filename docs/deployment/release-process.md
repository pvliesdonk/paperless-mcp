...
requirement pins one or you pass `--pre`, so `pip install pvliesdonk-paperless-mcp`
and `uv add pvliesdonk-paperless-mcp` still resolve to the newest stable. Ask for a
...
carrying the code. Its manifest launches `uvx --from pvliesdonk-paperless-mcp[all]==<version>`,
...
Every release attaches `paperless-mcp-plugin-<version>.zip`, and every
...
differently: it is a thin pointer that installs `pvliesdonk-paperless-mcp` from PyPI,
