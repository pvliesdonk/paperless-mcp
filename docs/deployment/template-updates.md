...
3. **Check the seeded-once files.** Copier's `_skip_if_exists` list names
   ...
   `.claude-plugin/**`, `src/paperless_mcp/tools.py`,
   `src/paperless_mcp/resources.py`, `src/paperless_mcp/prompts.py`,
   `src/paperless_mcp/domain.py`, `tests/conftest.py`,
   ...
   `src/paperless_mcp/static/app.src.html`.
