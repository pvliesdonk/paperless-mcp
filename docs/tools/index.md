...
Saved-view `show_on_dashboard` and `show_in_sidebar` are `null` when Paperless
omits them at payload version 10. Their values are unavailable. Python
consumers that assumed these fields were always `bool` must handle `None`.
...
permission and other failures do not trigger fallback. Restart the MCP server
after upgrading a connected 2.x instance to select version 10 again.

Version 10 task pages come from Paperless directly. Version 9 task arrays are
paginated locally. Both return the same page envelope and preserve the default
unacknowledged filter. Status filters and task-type names are translated for
the selected version; a task kind must exist on the connected instance.

Tasks include `task_type`, `trigger_source`, `result_data`,
`related_document_ids`, `date_started`, `duration_seconds` and
`wait_time_seconds` when available. Status values keep their uppercase spelling
for compatibility. The older `task_name`, `type`, `result`, `task_file_name`
and `related_document` fields remain available; the last field represents only
the first related document. Use the structured fields for complete results.
Version 9 cannot supply the richer timing and result data, so absent values
remain `null` rather than being inferred from result prose.
